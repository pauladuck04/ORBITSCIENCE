...
def home():
    ...
    likes = set(feedback.get("likes", []))
    dislikes = set(feedback.get("dislikes", []))
    
    papers_slice = df.iloc[start:start+per_page].copy()
    
    papers_list = []
    for _, row in papers_slice.iterrows():
        paper_data = row.to_dict()
        if row['title'] in likes:
            paper_data['voto'] = 'LIKE'
        elif row['title'] in dislikes:
            paper_data['voto'] = 'DISLIKE'
        else:
            paper_data['voto'] = None
